...
redEffectBtn.addEventListener('click', applyRedEffect);
rgbSplitBtn.addEventListener('click', applyRgbSplit);
greenScreenBtn.addEventListener('click', applyGreenScreen);

// Function to apply red effect
function applyRedEffect() {
    // Get the video frame, apply red effect, and update canvas
    let pixels = ctx.getImageData(0, 0, video.videoWidth, video.videoHeight);
    pixels = redEffect(pixels);
    ctx.putImageData(pixels, 0, 0);
}

// Function to apply RGB split
function applyRgbSplit() {
    // Get the video frame, apply RGB split, and update canvas
    let pixels = ctx.getImageData(0, 0, video.videoWidth, video.videoHeight);
    pixels = rgbSplit(pixels);
    ctx.putImageData(pixels, 0, 0);
}

// Function to apply green screen
function applyGreenScreen() {
    // Get the video frame, apply green screen, and update canvas
    let pixels = ctx.getImageData(0, 0, video.videoWidth, video.videoHeight);
    pixels = greenScreen(pixels);
    ctx.putImageData(pixels, 0, 0);
}
